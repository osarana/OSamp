...
#define music_path "C:/Users/oscar/source/repos/osamp/osamp/music.wav"

typedef struct
{
    SDL_Texture *texture; // DO NOT FREE IT
...
static SDL_bool load_skin(WinAmpSkin *skin, const char *fname)
{
    ...
    skin -> texmain = load_texture("src/hifi/Main.bmp");
    skin -> tex_cbuttons = load_texture("src/hifi/Cbuttons.bmp");
...
static void init_everything(int argc, char** argv)
{
    ...
    window = SDL_CreateWindow("Jadi's MP3", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, 275, 116, 0);
    if (!window)
    {
    ...
    SDL_Texture* skin_main = load_texture("src/hifi/Main.bmp");
    if (!skin_main)
    {
        panic_and_abort("Failed to load skin main.bmp", SDL_GetError());
    }

    SDL_Texture* skin_cbuttons = load_texture("src/hifi/Cbuttons.bmp");
    if (!skin_main)
    {
    ...
    SDL_EventState(SDL_DROPFILE, SDL_ENABLE); // it's disabled by default
...
static void draw_frame(SDL_Renderer *renderer, WinAmpSkin *skin)
{
    ...
    SDL_RenderCopy(renderer, skin -> texmain, NULL, NULL);
...
static SDL_bool handle_events(WinAmpSkin *skin)
{
    ...
    while (SDL_PollEvent(&e))
    {
        ...
        {
        case SDL_QUIT:
            return SDL_FALSE; // dont keep going
            break;
        ...
        case SDL_MOUSEMOTION: {
            ...
            else if (SDL_PointInRect(&pt, &balance_rect) && (e.motion.state & SDL_BUTTON_LMASK)) {
...
{
    init_everything(argc, argv); // come back
    #if 0
    ...
    #endif
    SDL_bool cbuttons_paused_pressed = SDL_FALSE;
